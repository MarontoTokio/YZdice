...
const initialDiceCounts = getInitialDiceCounts(initialSet);
const initialDiceById = getInitialDiceById(initialSet);
    ...
    changeDiceSet(diceSet) {
      set((state) => {
        const counts: DiceCounts = {
          red_d6: state.diceCounts.red_d6 || 0,
        };
        ...
        state.defaultDiceCounts = getInitialDiceCounts(diceSet);
        state.diceById = getInitialDiceById(diceSet);
      });
    },
    resetDiceCounts() {
      set((state) => {
...
function getInitialDiceCounts(diceSet: DiceSet): DiceCounts {
  return {
    ...getDiceCountsFromSet(diceSet),
    red_d6: 0,
  };
}
...
function getInitialDiceById(diceSet: DiceSet): Record<string, Die> {
  return {
    ...getDiceByIdFromSet(diceSet),
    red_d6: { id: "red_d6", type: "D6", style: "RED" as DiceStyle },
  };
}
